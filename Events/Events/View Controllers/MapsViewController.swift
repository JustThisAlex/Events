...
class MapsViewController: UIViewController, CLLocationManagerDelegate {
    ...
    private func addMap() {
        if CLLocationManager.authorizationStatus() == .authorizedWhenInUse ||
            ...
            geocoder.reverseGeocodeCoordinate(location.coordinate) { response, _ in
        ...
        let camera = GMSCameraPosition.camera(withLatitude: currentLocation?.coordinate.latitude ?? defaultLocation[0],
                                              ...
                                              zoom: 15)
        let frame = CGRect(x: view.bounds.minX, y: view.bounds.minY,
                           width: view.bounds.width, height: view.bounds.height - inset - 50)
    ...
    func addressEnded() {
        ...
        geocoder.geocodeAddressString(textField.text ?? "") { (address, error) in
            ...
            let alert = UIAlertController(title: "Please select your address", message: nil,
                                          preferredStyle: .actionSheet)
            alert.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: { _ in
                alert.dismiss(animated: true, completion: nil)
            }))
            for addr in address {
                alert.addAction(UIAlertAction(title: "\(addr.name ?? ""), \(addr.locality ?? "")",
                    ...
                    KeychainSwift.shared.set(addr.location?.coordinate.latitude.binade.description
                        ?? "", forKey: "latitude")
                    KeychainSwift.shared.set(addr.location?.coordinate.longitude.binade.description
                        ?? "", forKey: "longitude")
                    KeychainSwift.shared.set(addr.postalCode ?? "", forKey: "zipcode")
                    if self.isSelecting ?? false {
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "location"),
                                                        object: nil, userInfo:
                            ["loc": Location(street: addr.name,
                                              city: addr.locality,
                                              country: addr.country,
                                              latitude: addr.location?.coordinate.latitude.description,
                                              longitude: addr.location?.coordinate.longitude.description,
