...
struct Helper {
    ...
    static func login(email: String, password: String, viewController: UIViewController, segue: Bool = true) {
        chain.set(email, forKey: "email")
        chain.set(password, forKey: "password")
        ApiController().signIn(user: User(identifier: nil, email: email, username: nil,
                                          password: password, streetAddress: nil, city: nil,
                                          zipcode: nil, businessName: nil, latitude: nil,
                                          longitude: nil, country: nil)) { result in
            DispatchQueue.main.async {
                switch result {
                case .failure(let error):
                    if error == .userNotFound {
                        Helper.alert(on: viewController, "Your email address or password is wrong",
                                     "If you want to sign up choose that option above")
                    } else {
                        Helper.alert(on: viewController, "An error occured", "Please try again later")
                    }
                case .success:
                    chain.set(true, forKey: "authenticated")
                    guard segue else { return }
                    viewController.performSegue(withIdentifier: "FinishSegue", sender: nil)
                }
            }
        }
    }
    static func register(email: String, username: String, password: String, viewController: UIViewController) {
        chain.set(email, forKey: "email")
        chain.set(username, forKey: "username")
        chain.set(password, forKey: "password")
        ApiController().signUp(user: User(identifier: nil, email: email, username: username,
                                          password: password, streetAddress: address, city: city,
                                          zipcode: zipcode, businessName: nil, latitude: latitude,
                                          longitude: longitude, country: country)) { (result) in
            DispatchQueue.main.async {
                switch result {
                case .failure:
                    Helper.alert(on: viewController, "An error occured", "Please try again later")
    ...
    static func updateUser(newUsername: String?, newEmail: String?,
                           newPassword: String?, completion: @escaping () -> Void) {
        ...
        UserController().update(identifier: userID ?? ".",
                                email: checkedEmail,
